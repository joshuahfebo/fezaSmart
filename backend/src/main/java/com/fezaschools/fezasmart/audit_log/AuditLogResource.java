...
public class AuditLogResource {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<AuditLogDTO> getAuditLog(@PathVariable(name = "id") final Integer id) {
        return ResponseEntity.ok(auditLogService.get(id));
    }

    @PostMapping
    public ResponseEntity<Integer> createAuditLog(
            @RequestBody @Valid final AuditLogDTO auditLogDTO) {
        final Integer createdId = auditLogService.create(auditLogDTO);
        return new ResponseEntity<>(createdId, HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Integer> updateAuditLog(@PathVariable(name = "id") final Integer id,
            @RequestBody @Valid final AuditLogDTO auditLogDTO) {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteAuditLog(@PathVariable(name = "id") final Integer id) {
